...
func windows_os(ip_address string) {
	out, err := exec.Command("C:/Windows/System32/ping", "-n", "1", ip_address).Output()
	check_error(err)
...
func linux_os(ip_address string) {
	out, err := exec.Command("/usr/bin/ping", "-c", "1", ip_address).Output()
	check_error(err)
	output_ping := string(out[:])
	get_ttl(output_ping)
}
